...
def select_engineered_features(Mtr: pd.DataFrame, ytr: pd.Series, cfg: FeatureSelectCfg) -> List[str]:
    """Selection on ENGINEERED matrix (lags/rm/ema already built)."""
    M2 = _variance_filter(Mtr, cfg.variance_thresh)
    if cfg.mode == "manual":
        cols = [c for c in (cfg.manual_cols or []) if c in M2.columns]
        return cols
    corr = M2.apply(lambda s: s.corr(ytr), axis=0).abs().replace([np.inf, -np.inf], np.nan).fillna(0.0)
    if cfg.mode == "auto_topk":
        return corr.sort_values(ascending=False).head(cfg.topk).index.tolist()
    if cfg.mode == "auto_threshold":
        return corr[corr >= cfg.min_abs_corr].index.tolist()
